      ...
      <CardContent className="p-0">
        <Link href={`/product/${product.id}`}>
          <div className="relative">
            ...
            />
            {product.discount && <Badge className="absolute top-2 right-2 bg-red-600">{product.discount}% OFF</Badge>}
            {showNewBadge && product.isNewArrival && <Badge className="absolute top-2 left-2 bg-green-600">NEW</Badge>}
          </div>
        </Link>

        <div className="p-4">
          <Link href={`/product/${product.id}`}>
            <h3 className="font-semibold text-lg mb-2 hover:text-red-600 transition-colors line-clamp-2">
              {product.title}
            </h3>
          </Link>
